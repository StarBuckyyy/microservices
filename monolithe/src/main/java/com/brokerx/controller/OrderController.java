...
public class OrderController {
    ...
    public ResponseEntity<Map<String, Object>> getReservationStats() {
        ...
        com.brokerx.entity.Wallet firstWallet = null;

        try {
            List<com.brokerx.entity.Wallet> wallets = walletService.getAllWallets();
            if (!wallets.isEmpty()) {
                firstWallet = wallets.get(0);
            } else {
                logger.warn("No wallet found, attempting to create one for an active account.");
                List<com.brokerx.entity.Account> accounts = accountService.getAllAccounts();
                com.brokerx.entity.Account activeAccount = accounts.stream()
                    .filter(acc -> "ACTIVE".equals(acc.getStatus()))
                    .findFirst()
                    .orElse(null);

                if (activeAccount != null) {
                    firstWallet = walletService.createWallet(activeAccount);
                    logger.info("New wallet created for account: {}", activeAccount.getAccountId());
                }
            }

            if (firstWallet != null) {
                stats.put("walletId", firstWallet.getWalletId());
                stats.put("walletBalance", firstWallet.getBalance());
                stats.put("availableBalance", fundReservationService.getAvailableBalance(firstWallet));

                java.math.BigDecimal totalReserved = fundReservationService.getTotalReserved(firstWallet.getWalletId());
                stats.put("totalReservedForWallet", totalReserved);

                logger.info("Wallet stats retrieved: walletId={}, balance={}, available={}, reserved={}",
                        firstWallet.getWalletId(),
                        firstWallet.getBalance(),
                        fundReservationService.getAvailableBalance(firstWallet),
                        totalReserved);
            } else {
                logger.error("No active account found to create a wallet.");
                stats.put("walletBalance", null);
                stats.put("availableBalance", null);
                stats.put("totalReservedForWallet", java.math.BigDecimal.ZERO);
                stats.put("warning", "No active account or wallet found.");
            }

        } catch (Exception e) {
            logger.error("Error retrieving wallet info in reservation-stats", e);
            stats.put("error", "Error retrieving wallet information: " + e.getMessage());
        }
    ...
    public ResponseEntity<Map<String, Object>> debugInfo() {
        ...
        try {
            List<com.brokerx.entity.Wallet> wallets = walletService.getAllWallets();
            debug.put("totalWallets", wallets.size());
            
            if (!wallets.isEmpty()) {
                com.brokerx.entity.Wallet firstWallet = wallets.get(0);
                debug.put("firstWalletId", firstWallet.getWalletId());
                debug.put("firstWalletBalance", firstWallet.getBalance());
                debug.put("firstWalletCurrency", firstWallet.getCurrency());
                debug.put("firstWalletAccountId", firstWallet.getAccount().getAccountId());
                
                java.math.BigDecimal totalReserved = fundReservationService.getTotalReserved(firstWallet.getWalletId());
                java.math.BigDecimal availableBalance = fundReservationService.getAvailableBalance(firstWallet);
                
                debug.put("totalReserved", totalReserved);
                debug.put("availableBalance", availableBalance);
            }
